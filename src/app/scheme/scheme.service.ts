...
export class SchemeService extends ISchemeService {
  ...
  upload_file(item_id: number, file: File): Observable<any> {
    ...
    getChartData(date_from: number, date_to: number, data: string, limit: number = 1000, offset: number = 0, data_type: string = 'value', first_and_last = false): Observable<Paginator_Chart_Value> {
        let url = `/api/v2/scheme/${this.scheme.id}/chart_${data_type}/?ts_from=${date_from}&ts_to=${+date_to}&limit=${limit}&offset=${offset}&data=${data}&first_last=${+first_and_last}`;
    ...
    getChartParamData(date_from: number, date_to: number, data: string, limit: number = 1000, offset: number = 0, first_and_last = false): Observable<Paginator_Chart_Value> {
        return this.getChartData(date_from, date_to, data, limit, offset, 'param', first_and_last);
    }
